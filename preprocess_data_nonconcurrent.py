...
def process_mesh(mesh_filepath, target_filepath, executable, additional_args):
    ...
    command_with_spaces = [cmd_part + ' ' for cmd_part in command]
    command_with_spaces = ''.join(command_with_spaces)

    print('[HERE: In preprocess_data_nonconcurrent] EXECUTING %s' % command_with_spaces)
    os.system(command_with_spaces)
...
if __name__ == "__main__":
        ...
        for instance_dir in instance_dirs:
            ...
            try:
                ...
                meshes_targets_and_specific_args.append(
                    (
                        #os.path.join(shape_dir, mesh_filename), # redundant prefix
                        mesh_filename,
                        processed_filepath,
                        specific_args,
                    )
    ...
    for (mesh_filepath, target_filepath, specific_args,) in tqdm(meshes_targets_and_specific_args):
