...
int main(int argc, char** argv) {
  ...
  for (unsigned int v = 0; v < views.size(); v++) {
    ...
    // and
    // std::vector<Eigen::Vector4f> point_verts;
    // Q: Why are they 4f? A: 4f means 4 x 1.
  ...
  // these are the vertices and normals with only the first 3 coordinates
  // Q: but this is in main, so what is 'SampleFromSurface' (at the beginning) good for?
  // A: Perhaps these are a replacement for the original geometry, while 'SampleFromSurface' does the real surface sampling.
  ...
  // building kdtree for further queries
